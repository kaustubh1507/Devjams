   ...
   // height: 200,
    ...
    child:SingleChildScrollView(
      ...
      children: <Widget>[
        ...
        Center(
          child: Row(
            children: [
              ...
              IconButton(icon: Icon(Icons.mail), onPressed: () async=> await launchUrl(Uri.parse('mailto:$email?'))),
              IconButton(
        ...
        Text("${bioData}",style: TextStyle(color:Colors.black, fontWeight: FontWeight.bold, fontSize: 16)),
      ],
    ),) 
  ...
  void showPurchaseDialog(BuildContext context, String error) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          backgroundColor: Color.fromARGB(255, 149, 70, 196),
          elevation: 2,
          title: Text("ERROR", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold,color: Colors.red),),
          content: Text(error, style: TextStyle(fontSize: 18, color: Colors.white),),
          //buttons?
          actions: <Widget>[
            ElevatedButton(
             // style: ButtonStyle(),
